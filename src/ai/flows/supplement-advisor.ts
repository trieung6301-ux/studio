...
const SupplementAdvisorInputSchema = z.object({
  fitnessGoals: z
    .string()
    .describe('Mục tiêu thể chất của người dùng (ví dụ: xây dựng cơ bắp, giảm cân, tăng sức bền).'),
  dietaryRestrictions: z
    .string()
    .describe('Bất kỳ hạn chế nào về chế độ ăn uống của người dùng (ví dụ: ăn chay, thuần chay, không chứa gluten).'),
  workoutRoutine: z
    .string()
    .describe('Thói quen tập luyện điển hình của người dùng (ví dụ: cardio, cử tạ, crossfit).'),
});
...
const SupplementAdvisorOutputSchema = z.object({
  suggestedSupplements: z
    .string()
    .describe('Danh sách các thực phẩm bổ sung được đề xuất dựa trên thông tin đầu vào của người dùng.'),
  reasoning: z.string().describe('Lý do đằng sau các đề xuất thực phẩm bổ sung.'),
});
