...
import {
  type AdvisorState,
  getSupplementAdvice,
} from '@/actions/supplement-advisor'
        ...
        <form ref={formRef} action={formAction} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            {state.errors?.dietaryRestrictions && (
              <p className="text-sm font-medium text-destructive">
                {state.errors.dietaryRestrictions[0]}
              </p>
            )}
          </div>
          <div className="space-y-2">
            <Label htmlFor="workoutRoutine" className="text-lg font-semibold">
              Thói quen tập luyện
            </Label>
            <Textarea
              id="workoutRoutine"
            ...
            <CardContent className="space-y-4">
              <div>
                <h3 className="font-bold text-lg">
                  Thực phẩm bổ sung được đề xuất:
                </h3>
                <p className="text-muted-foreground whitespace-pre-wrap">
                  {state.data.suggestedSupplements}
                </p>
              </div>
              <div>
