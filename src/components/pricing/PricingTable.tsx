...
export function PricingTable() {
  ...
  const getPriceForPlan = (plan: typeof membershipPlans[0], cycle: 'monthly' | 'yearly') => {
    if (cycle === 'yearly') {
      const yearlyPlan = membershipPlans.find(p => p.id === `${plan.id.split('-')[0]}-yearly`);
      return yearlyPlan ? yearlyPlan.price : plan.price * 12 * 0.8; 
    }
    const monthlyPlan = membershipPlans.find(p => p.id === `${plan.id.split('-')[0]}-monthly`);
        ...
        {plansToShow.map((plan) => (
          ...
          >
            <CardHeader className="p-6 bg-card text-center">
              {plan.isPopular && (
                <Badge variant="secondary" className="w-fit self-center mb-4 bg-accent text-accent-foreground font-bold py-1 px-3 rounded-full">
                  <Zap className="w-4 h-4 mr-2"/>
                  Phổ biến nhất
                </Badge>
              )}
              <CardTitle className="text-3xl font-bold font-headline">{plan.title}</CardTitle>
              <CardDescription className="text-lg text-muted-foreground pt-1">{plan.description}</CardDescription>
            </CardHeader>
            <CardContent className="p-8 flex-grow">
              <div className="text-center mb-6">
                 <span className="text-5xl font-extrabold font-headline tracking-tighter">
                   {(getPriceForPlan(plan, billingCycle) / (billingCycle === 'yearly' ? 12 : 1)).toLocaleString('vi-VN')}₫
                  </span>
                 <span className="text-muted-foreground text-lg">/tháng</span>
                 {billingCycle === 'yearly' && (
                    <p className="text-sm text-green-600 font-semibold">Thanh toán hàng năm</p>
                 )}
              </div>
            ...
            <CardFooter className="p-6 mt-auto bg-secondary/30">
              <Button className="w-full text-lg py-6" variant={plan.isPopular ? 'default' : 'outline'}>
                Chọn gói
              </Button>
            </CardFooter>
          </Card>
